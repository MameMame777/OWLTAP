...
bool renderIlaGeneratedFiles(const IlaGeneratorConfig& config,
    ...
    const auto repo_root_rel = std::filesystem::relative(repo_root, out.output_dir);
    const std::string repo_root_rel_str = toForwardSlash(
        repo_root_rel.empty() ? std::filesystem::path(".") : repo_root_rel);
    ...
    create_project << "set repo_root [file normalize [file join $script_dir "
                   << repo_root_rel_str << "]]\n";
    create_project << "set rtl_dir [file normalize [file join $repo_root hdl ila rtl]]\n";
    ...
    create_project << "    [file join $rtl_dir ila_trigger.sv] \\\n";
    create_project << "    [file join $rtl_dir ila_capture_fsm.sv] \\\n";
    create_project << "    [file join $rtl_dir ila_bram.sv] \\\n";
    create_project << "    [file join $rtl_dir ila_bscane2_top.sv] \\\n";
    ...
    readme << "```\n\n";
    readme << "The helper scripts use relative paths back to the repository RTL under `hdl/ila/rtl`.\n";
